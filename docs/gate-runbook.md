...
- *the claude binary was replaced under the run (auto-update race)* or *Claude Code changed version mid-run* — Claude Code's auto-updater swapped its binary while cases were running; the later cases died with `EACCES ... posix_spawn ... claude` or ran on a different version than the earlier ones. The gate starts every child with `DISABLE_AUTOUPDATER=1` and compares the CLI version before and after the cases, but any *other* Claude Code session on the host can still replace the shared binary, so also set `"env": {"DISABLE_AUTOUPDATER": "1"}` in `~/.claude/settings.json` and restart long-running sessions; re-run with `--reuse-evals` so the cases that passed are kept.
